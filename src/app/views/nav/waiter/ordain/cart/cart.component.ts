...
export class CartComponent {
  ...
  public sumTotal(): void {
    this.productsCart.length === 0
      ...
      ? (this.total =
          this.productsCart[0].qty * this.productsCart[0].product.price)
      : (this.total = this.productsCart.reduce(
  ...
  public sendOrder() {
    ...
    this.HttpsService.post('orders', bodyHttp).subscribe({
  ...
  ngOnInit(): void {
    this.ServiceAdd.activatorAddToCart.subscribe((data) => {
      if (this.addToCart(data)) {
        //si es true podemos hacer push
